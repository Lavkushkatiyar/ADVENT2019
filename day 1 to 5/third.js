...
const shortest = (part1, part2) => {
  if (part1 + part2 < shortestStep) {
    shortestStep = part1 + part2;
  }
};
...
const move = {
  "L": (point) => {
    point.x -= 1;
    point.steps += 1;
    return [point.y, point.x, point.steps];
  },
  "R": (point) => {
    point.x += 1;
    point.steps += 1;
  ...
  for (let i = 0; i < arr1.length; i++) {
    for (let j = 0; j < arr2.length; j++) {
      if (arr1[i][0] === arr2[j][0] && arr1[i][1] === arr2[j][1]) {
        commonObject.push(arr1[i]);
        commonObject.push(arr2[j]);
      }
    }
  }
  return commonObject;
...
const getPoint = (arr1, direction, point) => {
  for (const a of direction) {
    arr1.push(move[a](point));
  }
};
const main = (input1, input2) => {
  const point1 = { steps: 0, x: 0, y: 0 };
  const point2 = { steps: 0, x: 0, y: 0 };
  const wire1 = parseInput(input1.split(","));
  const wire2 = parseInput(input2.split(","));

  for (const ins of wire1) {
    getPoint(arr1, ins, point1);
  }
  for (const ins of wire2) {
    getPoint(arr2, ins, point2);
  }
  const d = getCommonfrom();
  for (let i = 0; i < d.length; i = i + 2) {
    shortest(d[i][2], d[i + 1][2]);
  }
